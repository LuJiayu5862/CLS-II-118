...
| `0x14`    | `0x94`    | GET_GROUP_DICT    | 请求已注册组信息列表（含组名、描述、推荐频率）              |
...
| `0x23`    | `0xA3`    | WRITE_GROUP       | 按 GroupID 写整组；ACK_REQ 由 GROUP_ENTRY.CycleClass 推断  |
...
### 4.4c GET_GROUP_DICT 请求（CMD=0x14）
...
```
[EntryCount : BYTE  1B]   // 已注册组数
[Entry × EntryCount] :
  [GroupID    : BYTE  1B]   // 组ID（0x01~0xFE）
  [CycleClass : BYTE  1B]   // 组推荐写入频率，ACK_REQ 推断来源
...
> **未注册 GroupID 的缺省行为**：Name 显示为 `"Group_0xXX"`，Desc 为空，CycleClass 取组内所有变量 CycleClass 最保守值（fallback）。
...
Flags.ACK_REQ 置 1 时主站返回 WRITE_BY_ID_ACK（CMD=0xA1）。ACK_REQ 默认值由目标变量 CycleClass 推断（见 ADR 议题3），可由 `params.json` 在 ParamID 级覆盖。
...
Flags.ACK_REQ 默认值由 GROUP_ENTRY.CycleClass 推断（见 ADR 议题7），可由 `params.json` 在 GroupID 级覆盖。
...
> PARAM_ENTRY 与 GROUP_ENTRY 共用同一个 DictHash，握手时一并校验，无需新增 Hash 字段。
