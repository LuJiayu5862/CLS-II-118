...
| `0x13`    | `0x93`    | GET_GROUP_DICT    | 请求已注册组信息列表（含组名、描述、推荐操作频率）           |
| `0x14~0x1F` | —       | —                 | 预留                                                         |
...
| `0x23`    | `0xA3`    | WRITE_GROUP       | 按 GroupID 写整组（全量）；`Flags.ACK_REQ` 由上位机按操作类型直接设置         |
| `0x24~0x2F` | —       | —                 | 预留                                                                           |
...
[Entry × EntryCount] :
  ...
  [CycleClass : BYTE  1B]   // 变量物理更新周期：0=2ms,1=10ms,2=100ms,3=1s,4=手动；服务于读取侧（订阅间隔建议/轮询频率参考）
...
- **参与 Hash**：GroupID, NameLen, Name
- **不参与 Hash**：CycleClass, DescLen, Desc

> **设计取舍说明**：
> - PARAM_ENTRY 的 CycleClass 不参与 Hash：它描述变量的物理更新周期，仅服务于读取侧行为（订阅间隔建议、轮询频率参考），不影响任何协议写入行为，允许用户按实际需求调整而不触发参数表重拉。
> - GROUP_ENTRY 的 CycleClass 不参与 Hash（2026-05-17 修订）：原设计中它作为 WRITE_GROUP ACK_REQ 推断的唯一来源而参与 Hash；议题 3 修订后 ACK_REQ 改由上位机操作类型直接决定，CycleClass 退化为纯建议值，变更不再影响任何协议行为，故从 Hash 中移除。
