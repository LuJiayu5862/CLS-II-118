...
[ErrInfo  : UINT  2B]   // 附加信息（如非法 ParamID / GroupID 值）
```
...
> **设计取舍说明**：
> - PARAM_ENTRY 的 CycleClass 不参与 Hash，原因是它在协议层仅为建议值；上位机 `params.json` 中的 `"cycle"` 和 `"ack"` 字段才是运行时实际读写频率与 ACK 策略的依据，允许用户手动覆盖而不触发参数表重拉。
