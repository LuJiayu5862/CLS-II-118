...
**机制说明**：`Flags.ACK_REQ` 由上位机在发帧时动态置位，主站被动响应（收到 ACK_REQ=1 则回包，ACK_REQ=0 则静默）。适用于 WRITE_BY_ID；WRITE_GROUP 见议题 7。

**多 ParamID 混合 CycleClass 时**：取所有目标变量中 CycleClass 编号最大（最保守）者的默认 ACK 策略作为整帧推断结果。
...
```
GroupID    : BYTE  1B
CycleClass : BYTE  1B   // 组推荐写入频率，WRITE_GROUP ACK_REQ 推断唯一来源
NameLen    : BYTE  1B
Name       : NameLen B  // UTF-8，最长 32B
DescLen    : BYTE  1B
Desc       : DescLen B  // UTF-8，最长 64B
...
| 写指令 | ACK_REQ 默认推断来源 | 覆盖方式 |
|--------|-------------------|--------|
| WRITE_BY_ID | 目标 ParamID 的 CycleClass（混合时取最保守值） | `params.json` ParamID 级 `"ack"` |
...
**5. GROUP_ENTRY 纳入现有 DictHash 计算**
...
> GROUP_ENTRY 的 CycleClass **参与 Hash**（不同于 PARAM_ENTRY 的 CycleClass 不参与 Hash），原因是 GROUP_ENTRY.CycleClass 是 WRITE_GROUP ACK_REQ 推断的唯一来源，一旦变更必须触发上位机重拉以保证行为一致性。
